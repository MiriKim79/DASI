...
- **클라이언트는 원본 입력(답변/선택지 등)만 전송하고, 최종 나이/정신연령/점수는 항상 서버가 계산한다.** 클라이언트가 계산된 값을 직접 보내 저장시키는 API는 두지 않는다
...
- 서버는 제출된 answers만 받아 F1-2a(나이 추정)·F1-2b(정신연령)를 각각 계산해 함께 반환한다
- JWT가 있으면 이 요청 안에서 **서버가 계산한 값 그대로** 결과까지 저장한다(F1-4 참고) — 클라이언트가 별도로 나이/정신연령 값을 보내 저장을 요청하는 API는 없다
- 완료조건: 10개 답변 제출 시 나이/정신연령/결정적 답변 TOP3가 반환된다

#### F1-2a. 나이 추정 로직 (estimated_age) — [#5](../../issues/5)
- 정신연령과 **별개의 로직**으로 계산한다 (같은 가중평균식을 재사용하지 않음)
- 판단 기준: 문화/서비스를 실제 사용하기 시작한 시점, 연령 앵커 질문 응답, 당시 학생/성인/어린이였는지 등 생애주기 단서
- 정확한 산식은 1번이 설계·검증(F1-3)하며 반올림해 1세 단위로 출력한다

#### F1-2b. 정신연령 계산 로직 (mental_age) — [#6](../../issues/6)
- `mental_age = Σ(선택지 대표 나이 × 질문 가중치) / Σ(질문 가중치)`, 반올림 1세 단위
- 경험형 질문(7개)이 세대를 잘 구분할수록 가중치를 높게, 앵커 질문(3개)은 기준점 보정에 사용
- 결정적 답변 TOP3(`top_reasons`)도 이 로직에서 산출

### F1-3. 검증 — [#7](../../issues/7)
- 10~50대+ 가상 사용자 데이터로 estimated_age·mental_age 각각 일치율/±2세/±3세/MAE 측정 후 가중치 튜닝
- 완료조건: 검증 리포트 기준 두 로직의 가중치가 각각 1차 확정된다

### F1-4. 결과 저장 (F1-2에 통합) — [#9](../../issues/9)
- 별도의 저장 엔드포인트를 두지 않는다 — F1-2(`POST /api/age-test/submit`) 처리 중 JWT가 있으면 서버가 방금 계산한 값을 그대로 저장한다
- 클라이언트가 estimated_age/mental_age 값을 직접 실어 보내는 저장 요청은 만들지 않는다 (위변조 방지)
